...
public class ForStmtProcess {

	public void process(ForStmt n, ChangedFile changedFile, Map<Integer, String> changedFilePatchWLines, Map<String, String> repositoryJavaFiles) {
		...
		for(int key : changedFilePatchWLines.keySet()) {
			try {
				if (forLoopBegin <= key && key <= forLoopEnd) {
					...
					}.visit(n.getInitialization(), null);
					
					if(n.getCompare().get().isMethodCallExpr()) {
						...
						}.visit(n.getCompare().get().asMethodCallExpr(), null);
						
					}
					
					if(n.getCompare().get().isBinaryExpr()) {
						new VoidVisitorAdapter<Object>() {
						...
						}.visit(n.getCompare().get().asBinaryExpr(), null);
					}
